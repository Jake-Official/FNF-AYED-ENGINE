...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'New_BgAyed':
			// welp nothing to do just some flx sprite cuz that flxsprite
				var bgShit:BGSprite = new BGSprite('BackgroundShit/1light', -400, -200);
				bgShit.updateHitbox();
				bgShit.cameras = [camGame];
				bgShit.scrollFactor.set();
				
				var secondShit:BGSprite = new BGSprite('BackgroundShit/2ground', -400, -200);
				secondShit.updateHitbox();

				var shaderImg:BGSprite = new BGSprite('BackgroundShit/3building', -400, -200);
				shaderImg.updateHitbox();
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'textLyrics':
				...
				var textLyrics:FlxText = new FlxText(0, 0, 0, "", 32);
				textLyrics.color = FlxColor.CYAN;
				textLyrics.cameras = [camHUD];
				textLyrics.scrollFactor.set();
				textLyrics.text = textS;
				textLyrics.setFormat("VCR OSD Mono", 32);
				textLyrics.screenCenter(X);
				textLyrics.y = 450;

				if(value1 != null){
					FlxG.log.warn("what the fuck did that already started added yet ?");
				}else{
					FlxG.log.warn("stay fucking calm without that event shit");
				}

				if(value2 == "remove"){
					trace("Function Running Of Removing The Object");
